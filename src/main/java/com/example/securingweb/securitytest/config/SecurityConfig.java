...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(request -> request.regexMatchers("/api/v1/auth/*").permitAll().requestMatchers().hasAnyAuthority(Role.ADMIN.name()).requestMatchers().hasAnyAuthority(Role.USER.name()).anyRequest().authenticated())
